...
import com.udesk.mapper.AddressMapper;
...
public class AddressServiceImpl 
	...
	@Transactional
	public void deleteById(Integer uid, Integer id) throws AddressNotFoundException, ArgumentException, UpdateDataException {
	 ...
	 */
	private void insert(
		String currentUser, Address address) throws InsertDataException {
		// 添加日志数据
		Date now = new Date();
	...
	private Integer setNonDefault(Integer uid) {
		...
		if (rows == 0) {
			throw new UpdateDataException("更新收货地址数据时发生未知错误！请联系系统管理员！");
		}
		return rows;
	}
